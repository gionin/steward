...
4. **Delete containers.** Awaiting test. The engine already supports a guarded, cascading delete: a protected container refuses, and deleting a normal container removes its tasks, routines, and their day-log records. Built 2026-06-24: a ✕ affordance on every non-protected container row, with a confirm that names what cascades (its N tasks and M routines and their history); the protected Misc shows none. Resolved with the lead designer as hard-delete-with-prompt, no separate archive — the container status sections (Set aside / Completed / Cancelled) already serve that role. Wired to the existing `delete_container` command. Awaiting hands-on confirmation.
...
15. **Add containers with the same "+" affordance as tasks and routines.** Awaiting test. Containers are added through a separate "New area" form at the bottom of the Containers view (a name field plus an "Add container" button), inconsistent with the inline "+ add routine" affordance and the task "+". Replace it with the same inline "+ add container" pattern so adding an area matches adding a task or routine. UI-only; the `add_container` bridge command already exists. Built 2026-06-24: the bottom "New area" form is gone, replaced by an inline "+ add container" on every status section, mirroring "+ add routine". Awaiting hands-on confirmation.

16. **Data peace-of-mind: show where the data lives, and one-click backup.** Awaiting test. State is a single SQLite file at `~/.steward/steward.db` (on the lead designer's machine, `C:\Users\Pichau\.steward\steward.db`) — outside the project folder and the repo. The app never shows this, which feeds a fear of losing everything. Surface the exact path in Settings, add an "open data folder" affordance, and add a one-click "Back up database now" that copies the live file to a timestamped `.db` (the existing context export is AI text, not a backup). Needs a small bridge method to copy the file; the rest is UI. Built 2026-06-24: Settings now shows the DB path with an "Open folder" button and a one-click "Back up" that writes a timestamped copy via SQLite's own backup API (consistent even mid-write); covered by a new bridge test. Awaiting hands-on confirmation.

17. **Today and History as two views of one per-day record (the navigable day lens).** Awaiting test (phases a, b). Today and History are two lenses on the same per-day record: a day's items are the same `DayItem` objects in both. Today is the single-day lens; it opens on the current day and steps day to day with prev/next and a jump-to-today control, retiring "Start new day". History becomes the calendar overview of the same records. Nothing is frozen; every day, the past included, is editable. Schedule direction is the **Record model**: the derivation paints the current day and any future day reached by navigating ahead, where it shows scheduled **routines only** (open Ready tasks are not projected ahead; they live on the current day and roll forward only as it advances). Past days keep the record of what was actually drafted and checked and are never re-derived, so changing a routine never rewrites an earlier day; a skipped past day can be reconstructed on demand by pulling its scheduled routines in. An unchecked item has **no failure framing** (no red): past reads as simply not done, future as still to come. Backend plan: keep `get_state(liveKey)` syncing the live day (the client sends the real system date); add a viewed-day path that syncs only when viewed == live, derives routines-only for future days without persisting, and returns past days as their stored record; plus a back-fill command for a skipped past day. Build phased: (a) the day model + navigable Today lens; (b) History as the calendar; (c) add/remove tasks on a future day, not checkable. Built 2026-06-24 (phase a): the backend viewed-vs-live derivation (the live day syncs; a past day is its stored record and is never re-derived; a future day is a routines-only preview that is not persisted) plus a back-fill command for a skipped past day, and the Today lens (a date header with prev/next and jump-to-today, neutral rendering with no red, the routines-only future preview, and the retired "Start new day"). Four new headless bridge checks pass and the embedded JS syntax-checks clean; the UI is asserted, awaiting hands-on confirmation. Phase (b) built 2026-06-24: History is now a month calendar (prev/next month, a This-month jump, done/total per day); clicking a day opens it in the checklist lens, so per-day editing lives in the lens (the log-past-activity form is kept). The nav tabs were renamed Today to Checklist and History to Calendar. Phase (c), add/remove tasks on a future day (not checkable), is next and supersedes item 18.

18. **Schedule a task to enter Ready on a future day.** Folded into item 17 phase (c) (add/remove tasks on a future day, not checkable) — settled with the lead designer 2026-06-24. A way to set, ahead of time, that a specific task should drop into the Ready column on a chosen future date (distinct from routines, which recur by weekday). Raised by the lead designer during the item 17 design and deliberately deferred to its own design pass.
...
- Built the first batch from that review — items 4, 15, 16, now all **Awaiting test**. (4) Container deletion: a ✕ on each non-protected container row with a cascade-naming confirm, wired to the existing engine delete; decided with the lead designer as hard-delete-with-prompt, no separate archive (the status sections serve that), so the delete-UX decision is resolved. (15) Containers gained the inline "+ add container" affordance, retiring the old bottom form. (16) Settings now surfaces the data file path, an "Open folder" button, and one-click timestamped backup via SQLite's backup API. Added a bridge regression test for data-location + backup; all suites pass and the embedded JS syntax-checks clean. The UI (delete flow, the "+", the Settings panel) is asserted, not yet run — awaiting the lead designer's confirmation on a real launch.
- Container-view refinements from the lead designer's review of that batch (still part of item 15, Awaiting test): removed the per-row status dropdown — dragging between the labelled sections already restatuses; made the whole container row draggable, not just the ⋮⋮ handle; and put the "+ add container" on every status section, where adding in a section creates the container already in that status (`add_container` gained an optional status, covered by a new test). Suites pass, JS syntax-checks clean.
- Designed item 17 with the lead designer and settled it (now Ready). Corrected the model: Today and History are two views of one per-day record (the same `DayItem`), nothing is frozen, every day editable. Decisions: no failure framing for unchecked items (no red); future days project routines only; the Record schedule direction (forward-only, past stays the truthful record, back-fill on demand). Wrote the mechanics into Architecture sections 5 and 6. Registered item 18 (schedule a task into Ready on a future day) as a separate, deferred feature. No app code yet; build is phased: (a) day model + navigable Today lens, (b) History calendar.
- Built item 17 phase (a), the navigable day lens. Backend now separates the live day (which syncs) from the viewed day: a past day is its stored record and is never re-derived; a future day is a routines-only preview that is not persisted; plus a `backfill_day` command to reconstruct a skipped past day. `today_key` now always means the live day, with a `viewed_key` riding along on the day-scoped commands. The Today view gained a date header with prev/next and jump-to-today, neutral rendering (no red), the future preview, and lost "Start new day". Four new bridge tests pass; all suites green; JS syntax-checks clean. The UI is asserted, not yet run; phase (b), the History calendar, is next.
- Built item 17 phase (b), the calendar, plus two small fixes the lead designer asked for. History is now a month grid (prev/next month, a This-month jump, done/total per day) that opens a clicked day in the checklist lens; per-day editing moved fully into the lens and the log-past-activity form was kept. The checklist date is now fixed-width so the prev/next arrows stay put, and the nav tabs were renamed Today to Checklist and History to Calendar. Defined phase (c): add/remove tasks on a future day (not checkable), which supersedes item 18. JS syntax-checks clean; all suites green (backend untouched). Phase (c) is next, after a hands-on pass.
